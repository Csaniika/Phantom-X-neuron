...
int BioloidController::getPose(int id){
    for(int i=0; i<poseSize; i++){
		pose_[i] = ax12GetRegister(id_[i],AX_PRESENT_POSITION_L,2)<<BIOLOID_SHIFT;
        if( id_[i] == id )
...
void BioloidController::relaxServos(){
	for(int i=0; i<poseSize; i++){
		ax12SetRegister(id_[i], AX_TORQUE_ENABLE, 0);
    }
}
...
void BioloidController::setNextPose2(int id, int pos){
    for(int i=0; i<poseSize; i++){
        if( id_[i] == id ){
